'use client'

import Link from 'next/link'
import { Navigation } from '@/components/navigation'
import { Button } from '@/components/ui/button'

export default function LandingPage() {
  const steps = [
    {
      step: '01',
      title: 'Describe your project',
      description: 'Write your idea in plain language — no boilerplate or tech stack details needed.',
    },
    {
      step: '02',
      title: 'Choose language & type',
      description: 'Pick Python, JavaScript, or TypeScript and select the kind of application you want to build.',
    },
    {
      step: '03',
      title: 'Let it generate',
      description: 'Prompt2Product turns your description into a complete, production-ready codebase.',
    },
    {
      step: '04',
      title: 'Preview & test',
      description: 'Inspect the generated project, run it locally, and explore the structure.',
    },
    {
      step: '05',
      title: 'Download or refine',
      description: 'Download the code, or request changes and iterate until it matches your vision.',
    },
  ]

  return (
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden">
      <Navigation />

      <main className="pt-24">
        <div className="mx-auto max-w-5xl px-4 md:px-6">
          {/* Hero Section */}
          <section className="min-h-[calc(100vh-6rem)] flex flex-col justify-center gap-10 md:gap-12">
            {/* Row 1 - Hero copy + CTA */}
            <div className="space-y-8 animate-in fade-in duration-1000 text-center">
              <div>
                <h1 className="text-4xl md:text-6xl font-bold leading-tight text-foreground mb-4 text-balance">
                  Turn Natural Language Into Executable Code
                </h1>
                <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto font-light">
                  Describe your idea in plain English. Get complete, production-ready code within minutes.
                </p>
              </div>

              <div className="flex justify-center gap-4 pt-2 flex-wrap">
                <Link href="/describe">
                  <Button
                    size="lg"
                    className="btn-glow bg-primary hover:bg-primary/85 text-primary-foreground font-semibold shadow-lg hover:shadow-xl"
                  >
                    Start Building →
                  </Button>
                </Link>
              </div>
            </div>

            {/* Row 2 - Horizontal 5-step flow */}
            <div className="animate-in fade-in duration-1000 delay-150 text-center">
              <h2 className="text-base md:text-lg font-semibold text-foreground mb-4">
                Your project in <span className="text-primary">5 easy steps</span>
              </h2>

              <div className="grid grid-cols-1 md:grid-cols-5 gap-3 md:gap-4">
                {steps.map((item) => (
                  <div
                    key={item.step}
                    className="rounded-xl bg-card border border-border/70 px-3 py-3 md:px-3.5 md:py-4 flex flex-col gap-1.5 text-center items-center"
                  >
                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/20 text-[11px] font-semibold text-primary mb-1">
                      {item.step}
                    </div>
                    <p className="text-xs md:text-sm font-medium text-foreground">
                      {item.title}
                    </p>
                    <p className="text-[11px] md:text-xs text-muted-foreground leading-snug">
                      {item.description}
                    </p>
                  </div>
                ))}
              </div>
            </div>
          </section>
        </div>
      </main>
    </div>
  )
}
