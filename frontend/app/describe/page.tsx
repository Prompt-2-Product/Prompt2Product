...
export default function DescribePage() {
  ...
  useEffect(() => {
    const checkHealth = async () => {
      try {
        console.log("[DESCRIBE] Checking backend health...")
        const res = await fetch(`${api.projects.create.toString().includes('127.0.0.1') ? 'http://127.0.0.1:8002' : 'http://10.1.129.232:8002'}/`)
        if (res.ok) {
          toast.success("Backend Connected", { description: "API is ready on port 8002" })
        } else {
          toast.error("Backend Error", { description: `Status: ${res.status}` })
        }
      } catch (err) {
        console.error("[DESCRIBE] Health check failed:", err)
        toast.error("Backend Offline", {
          description: "Cannot reach port 8002. Please check your network/firewall.",
          duration: 10000
        })
      }
    }
  ...
  const handleGenerateProject = async () => {
    if (description.trim()) {
      ...
      console.log("[DESCRIBE] Creating project...")
      try {
        toast.loading("Creating project...", { id: "gen-toast" })
        ...
        console.log("[DESCRIBE] Project created:", project)
        toast.loading("Starting generation pipeline...", { id: "gen-toast" })

        // 2. Start the run
        const run = await api.projects.startRun(project.id, description)
        console.log("[DESCRIBE] Run started:", run)
        toast.success("Generation started! Redirecting...", { id: "gen-toast" })
        ...
        setTimeout(() => router.push('/generating'), 1000)
      } catch (error) {
        console.error('Failed to generate project:', error)
        toast.error('Failed to connect to backend on port 8002', { id: "gen-toast" })
      } finally {
